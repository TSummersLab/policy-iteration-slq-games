...
def get_problem_data(problem_type, problem_data_id=None, seed=None):
    ...
    if problem_type == 'load':
        ...
        problem_data = gen_rand_problem_data(n=3, m=2, p=1, rho=0.9, seed=seed)
...
if __name__ == "__main__":
    seed = 6
    npr.seed(seed)
    problem_data = get_problem_data(problem_type='gen', seed=seed)
    problem_data_keys = ['A', 'B', 'C', 'Ai', 'Bj', 'Ck', 'varAi', 'varBj', 'varCk', 'Q', 'R', 'S']
    A, B, C, Ai, Bj, Ck, varAi, varBj, varCk, Q, R, S = [problem_data[key] for key in problem_data_keys]
    n, m, p = [M.shape[1] for M in [A, B, C]]
    q, r, s = [M.shape[0] for M in [Ai, Bj, Ck]]
    ...
    QRS = sla.block_diag(Q, R, -S)
    RS = sla.block_diag(R, -S)
    ...
    QuvQvvinv0 = solveb(Qvu0.T, Qvv0)
    QvuQuuinv0 = solveb(Qvu0, Quu0)

    # Policy improvement
    K1 = -la.solve(Quu0-QuvQvvinv0.dot(Qvu0), Qux0-QuvQvvinv0.dot(Qvx0))
    L1 = -la.solve(Qvv0-QvuQuuinv0.dot(Qvu0.T), Qvx0-QvuQuuinv0.dot(Qux0))


    P1 = gdlyap(problem_data, K1, L1)
    H1 = QRS + mdot(ABC.T, P1, ABC)

    P = P_pi
    H = QRS + mdot(ABC.T, P, ABC)
    ...
    IKL0 = np.vstack([np.eye(n), K0, L0])
    IKL1 = np.vstack([np.eye(n), K1, L1])
    IKL_pi = np.vstack([np.eye(n), K_pi, L_pi])

    KL0 = np.vstack([K0, L0])
    KL1 = np.vstack([K1, L1])
    KL_pi = np.vstack([K_pi, L_pi])

    KL10_diff = KL1-KL0

    BC = np.hstack([B, C])

    print("Required assumption -S + mdot(C.T, P, C) > 0 : %s" % (is_pos_def(-S + mdot(C.T, P, C))))



    Z = mdot(IKL1.T, H0, IKL1) + mdot(KL10_diff.T, H0[n:, n:], KL10_diff)


    # Z0 == P0
    Z0 = mdot(IKL0.T, H0, IKL0)
    Z1 = mdot(IKL1.T, H0, IKL1)
    Z_pi = mdot(IKL_pi.T, H0, IKL_pi)


    # LHS = mdot(IKL0.T, H0, IKL0) - mdot(IKL1.T, H0, IKL1)
    # RHS = mdot((KL1-KL0).T, H0[n:, n:], KL1-KL0)

    # LHS = mdot(IKL_pi.T, H0, IKL_pi) - mdot(IKL1.T, H0, IKL1)
    # RHS = mdot((KL1-KL_pi).T, H0[n:, n:], KL1-KL_pi)

    # LHS = mdot(IKL1.T, H, IKL1) - mdot(IKL_pi.T, H, IKL_pi)
    # RHS = mdot((KL1-KL_pi).T, H[n:, n:], KL1-KL_pi)

    # LHS = mdot(IKL0.T, H, IKL0) - mdot(IKL_pi.T, H, IKL_pi)
    # RHS = mdot((KL0-KL_pi).T, H[n:, n:], KL0-KL_pi)

    i = 3
    IKLi = np.vstack([np.eye(n), K_history_pi[i], L_history_pi[i]])
    KLi = np.vstack([K_history_pi[i], L_history_pi[i]])

    LHS = mdot(IKLi.T, H, IKLi) - mdot(IKL_pi.T, H, IKL_pi)
    RHS = mdot((KLi-KL_pi).T, H[n:, n:], KLi-KL_pi)
    ...
    # la.norm(P_history_pi - P_pi, ord=2, axis=0)


    # M = np.zeros([n, n])
    # Z = np.zeros([n, n])
    # for i in range(100):
    #     Y = la.matrix_power(A+B@K1+C@L1, i)
    #     Z += mdot(Y.T, (KL1-KL0).T, H0[n:, n:], KL1-KL0,  Y)
    # LHS = P0-P1
    # RHS = Z
    # print(LHS)
    # print(RHS)
    ...
    # Z = np.zeros([n, n])
    # for i in range(100):
    #     Y = la.matrix_power(A+B@K0+C@L0, i)
    #     Z += mdot(Y.T, (KL0-KL_pi).T, H[n:, n:], KL0-KL_pi,  Y)
    # LHS = P0-P_pi
    # RHS = Z
    # print(LHS)
    # print(RHS)
    ...
    Z = np.zeros([n, n])
    for i in range(100):
        Y = la.matrix_power(A + B@Ki + C@Li, i)
        Z += mdot(Y.T, (KLi-KL_pi).T, H[n:, n:], KLi-KL_pi, Y)
    LHS = Pi - P_pi
    RHS = Z
    print(LHS)
    print(RHS)
    ...
    #
    # LHS = P0
    # # RHS = mdot(IKL1.T, H0, IKL1) + mdot(KL10_diff.T, H0[n:, n:], KL10_diff)
    # # RHS = mdot(IKL1.T, QRS, IKL1) + mdot(IKL1.T, ABC.T, P0, ABC, IKL1) + mdot(KL10_diff.T, H0[n:, n:], KL10_diff)
    # # RHS = mdot(IKL1.T, ABC.T, P0, ABC, IKL1) + mdot(IKL1.T, QRS, IKL1) + mdot(KL10_diff.T, H0[n:, n:], KL10_diff)
    # # RHS = mdot(IKL1.T, ABC.T, P0, ABC, IKL1) + Q + mdot(KL1.T, RS, KL1) + mdot(KL10_diff.T, H0[n:, n:], KL10_diff)
    # RHS = mdot(IKL1.T, ABC.T, P0, ABC, IKL1) + Q + mdot(KL1.T, RS, KL1) + mdot(KL10_diff.T, RS, KL10_diff) + mdot(KL10_diff.T, BC.T, P0, BC, KL10_diff)
    # M = mdot(IKL1.T, QRS, IKL1) + mdot(KL10_diff.T, H0[n:, n:], KL10_diff)
    # print(LHS)
    # print(RHS)
    # print(LHS-RHS)
    ...
    # checking identity in Hewer, Bu about gain difference == infinite sum


    K0 = np.zeros([m, n])

    AK0 = A+B@K0
    QK0 = Q + mdot(K0.T, R, K0)
    ...
    K1 = -la.inv((R+mdot(B.T, P0, B))) @ mdot(B.T, P0, A)
    AK1 = A+B@K1
    QK1 = Q + mdot(K1.T, R, K1)
    ...
    Z = np.zeros([n, n])
    for i in range(100):
        Y = la.matrix_power(A + np.dot(B, K), i)
        M = mdot(Y.T, (Ki-K).T, R + mdot(B.T, P, B), Ki-K, Y)
        Z += M
    ...
    #
    # P1_vi = Q + mdot(A.T, P0, A) - mdot(A.T, P0, B, la.inv((R+mdot(B.T, P0, B))), B.T, P0, A)
